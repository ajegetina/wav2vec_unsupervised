...
class RealData:
    ...
    Real samples are one-hot phoneme vectors built from the unpaired text
    corpus that has been phonemized and batched by the fairseq data pipeline
    (arriving as ``random_label`` in each training batch).

    The discriminator never sees audio — it only sees:
      (a) one-hot phoneme vectors from this class  (real)
    ...
    def to_one_hot(
        ...
        """
        Convert a batch of token indices to one-hot phoneme vectors.

        Args:
            tokens           : [B, T]  long tensor of phoneme indices
            reference_tensor : any tensor on the target device/dtype

        Returns:
            token_x : [B, T, output_dim]  float one-hot tensor
        """
        token_x = reference_tensor.new_zeros(tokens.numel(), self.output_dim)
...
class Generator(nn.Module):
    ...
    def forward(self, dense_x, dense_padding_mask):
        """
        Args:
            dense_x           : [B, T, input_dim]  audio feature vectors
            dense_padding_mask: [B, T]  bool tensor, True where padded

        Returns dict with:
            dense_x           : [B, T, output_dim]  raw phoneme logits
            dense_padding_mask: [B, T]  (adjusted for stride if > 1)
            inter_x           : [B, T, input_dim]  intermediate features
                                (only present when cfg.generator_residual=True,
                                used for the optional MMI loss)
        """
        result = {}
...
# ---------------------------------------------------------------------------
# Standalone loss functions (called from Wav2vec_U.forward)
# ---------------------------------------------------------------------------
...
def calc_gradient_penalty(
    ...
    probabilistic_slicing: bool = False,
) -> torch.Tensor:
    """
    WGAN-GP gradient penalty on interpolations between real and fake samples.

    Encourages the discriminator to be 1-Lipschitz, stabilising training
    ...
    Args:
        ...
        real_data            : [B, T, V]  real phoneme one-hot vectors
        fake_data            : [B, T, V]  generated phoneme soft vectors
        probabilistic_slicing: if True, randomly slice to a common (B, T)
                               instead of truncating

    Returns:
        gradient_penalty : [B]  per-sample penalty (caller sums and weights)
    """
    b_size = min(real_data.size(0), fake_data.size(0))
    t_size = min(real_data.size(1), fake_data.size(1))

    if probabilistic_slicing:
    ...
    disc_interpolates = discriminator(interpolates, None)
...
def discriminator_loss(
    ...
    """
    Adversarial loss for the discriminator step.

    Discriminator tries to output 1 for real (token_y) and 0 for fake (dense_y).
    Label smoothing is applied to both targets.

    Returns:
        loss_dense : loss on fake (generated) samples
        loss_token : loss on real (text-derived) samples
    """
...
def generator_loss(
    ...
    """
    Adversarial loss for the generator step.

    Generator tries to fool the discriminator into outputting 1 for fake samples.
    """
...
def smoothness_loss(
    ...
    Padded positions are zeroed out before averaging.
    """
    loss = F.mse_loss(dense_logits[:, :-1], dense_logits[:, 1:], reduction="none")
    loss[dense_padding_mask[:, 1:]] = 0
    return loss.mean() * sample_size * weight


def code_penalty(
    prob_perplexity: torch.Tensor,
    num_vars: int,
    sample_size: int,
    weight: float,
) -> torch.Tensor:
    ...
    When all phonemes are used equally, perplexity == |V| and penalty == 0.
    ``prob_perplexity`` is pre-computed by ``Wav2vec_U.normalize()``.
    """
    return ((num_vars - prob_perplexity) / num_vars) * sample_size * weight
...
def mmi_loss(
    ...
    decoder: nn.Module,
    target_downsample_rate: int,
    weight: float,
) -> torch.Tensor:
    """
    Maximum Mutual Information loss between intermediate generator features
    and a downsampled auxiliary target sequence.

    Only active when ``cfg.mmi_weight > 0`` and an ``aux_target`` is provided.
    """
    inter_x = decoder(inter_x)
    if target_downsample_rate > 1:
        aux_target = aux_target[:, ::target_downsample_rate]
    ...
    return loss.mean() * loss.shape[0] * weight


@register_model("wav2vec_u", dataclass=Wav2vec_UConfig)
class Wav2vec_U(BaseFairseqModel):
    def calc_gradient_penalty(self, real_data, fake_data):
        """Delegates to the module-level ``calc_gradient_penalty`` function."""
        return calc_gradient_penalty(
            self.discriminator,
            real_data,
            fake_data,
            self.cfg.probabilistic_grad_penalty_slicing,
        )
    ...
    def __init__(self, cfg: Wav2vec_UConfig, target_dict):
        ...
        self.real_data = RealData(output_size)
    ...
    def forward(
        ...
        # Generator produces fake phoneme logits from audio features
        ...
        # RealData converts text token indices → one-hot phoneme vectors
        token_x = (
            self.real_data.to_one_hot(random_label, features)
        ...
        if d_step:
            loss_dense, loss_token = discriminator_loss(
                dense_y, token_y, fake_smooth, real_smooth
            )
            ...
            grad_pen = None
            loss_token = None
            loss_dense = generator_loss(dense_y, fake_smooth)

            num_vars = dense_x.size(-1)
            if prob_perplexity is not None:
                code_pen = code_penalty(
                    prob_perplexity, num_vars, sample_size, self.code_penalty
                )

            if self.smoothness_weight > 0:
                smooth_loss = smoothness_loss(
                    dense_logits, dense_padding_mask, sample_size, self.smoothness_weight
                )
